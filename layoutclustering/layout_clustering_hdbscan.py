...
def _select_relevant_local_element_ids(element_ids, indexed_elements, bbox, center_xy=None, max_elements=None):
    candidate_ids = []
    seen = set()
    for raw_elem_id in element_ids or []:
        try:
            elem_id = int(raw_elem_id)
        except (TypeError, ValueError):
            continue
        if elem_id in seen or elem_id < 0 or elem_id >= len(indexed_elements):
            continue
        item_bbox = indexed_elements[elem_id].get("bbox")
        if item_bbox is None or _bbox_intersection(item_bbox, bbox) is None:
            continue
        seen.add(elem_id)
...
def _approx_clip_local_elements_to_bbox(element_ids, indexed_elements, bbox, center_xy=None, max_elements=None):
    polygons = []
    candidate_ids = _select_relevant_local_element_ids(
        element_ids,
        indexed_elements,
        bbox,
        center_xy=center_xy,
...
def _build_shift_candidate_record(origin_idx, origin_record, center, indexed_elements, local_element_ids, *,
    ...
    outer_polygons = _approx_clip_local_elements_to_bbox(
        local_element_ids,
        indexed_elements,
        outer_bbox,
        center_xy=center,
...
def _generate_pattern_shift_candidates_for_record(origin_idx, origin_record, spatial_index, indexed_elements,
                                                  ...
                                                  neighbor_limit=128, max_events_per_direction=3):
    ...
    pending_shifts = []
    pending_center_keys = {(round(base_center[0], 6), round(base_center[1], 6))}
    for direction in _normalize_shift_directions(shift_directions):
        for dx, dy in directional_shifts.get(direction, []):
            ...
            if center_key in pending_center_keys:
                continue
            pending_center_keys.add(center_key)
            pending_shifts.append((new_center, float(dx), float(dy)))

    if not pending_shifts:
        return candidates

    shifted_outer_bboxes = [
        _make_centered_bbox(new_center, outer_side, outer_side)
        for new_center, _, _ in pending_shifts
    ]
    union_bbox = (
        min(float(bbox[0]) for bbox in shifted_outer_bboxes),
        min(float(bbox[1]) for bbox in shifted_outer_bboxes),
        max(float(bbox[2]) for bbox in shifted_outer_bboxes),
        max(float(bbox[3]) for bbox in shifted_outer_bboxes),
    )

    local_element_ids = list(spatial_index.intersection(union_bbox))

    for new_center, dx, dy in pending_shifts:
...
class LayoutClusteringPipeline:
    ...
    def _init_runtime_state(self) -> None:
        ...
        self.cluster_review_path_map = {}
        self._owned_temp_dir = None

    def _review_path_key(self, filepath: str) -> str:
        if not filepath:
            return ""
        try:
            path = _resolve_fs_path(filepath)
            ...
            return os.path.normcase(str(filepath))

    def _result_filepath(self, filepath: str, remap_review_paths: bool = True) -> str:
        path_text = str(filepath)
        if not remap_review_paths or not self.cluster_review_path_map:
            return path_text
        return self.cluster_review_path_map.get(self._review_path_key(path_text), path_text)
    ...
    def get_results(self, remap_review_paths: bool = True) -> Dict[str, Any]:
        """获取聚类结果"""
        ...
        for i, (cluster, rep_idx) in enumerate(zip(self.clusters, self.representatives)):
            ...
            cluster_files = [
                self._result_filepath(self.filepaths[idx], remap_review_paths=remap_review_paths)
                for idx in valid_cluster_indices
            ]
            representative_file = self._result_filepath(
                self.filepaths[rep_idx],
                remap_review_paths=remap_review_paths,
            )
            cluster_metadata = [
                self.sample_infos[idx] if idx < len(self.sample_infos) else None
                for idx in valid_cluster_indices
        ...
        result = {
            ...
            "file_list": [
                self._result_filepath(filepath, remap_review_paths=remap_review_paths)
    ...
    def export_cluster_review(self, output_dir: str) -> Dict[str, Any]:
        ...
        result = self.get_results(remap_review_paths=False)
        ...
        review_root = _resolve_fs_path(output_dir)
        ...
        for cluster in clusters:
            ...
            rep_export_path = None
            for member_idx, src in enumerate(cluster.get("sample_files", [])):
                src_path = _resolve_fs_path(src)
                if not src_path.exists():
                    missing_files.append(str(src_path))
                    continue
                src_key = self._review_path_key(src)
                rep_key = self._review_path_key(rep_path)
                prefix = "REP__" if src_key == rep_key else "sample__"
                dest_name = f"{prefix}{member_idx:04d}__{src_path.name}"
                dest_path = (cluster_dir / dest_name).resolve()
                shutil.copy2(src_path, dest_path)
                path_map[src_key] = str(dest_path)
                if src_key == rep_key:
                    rep_export_path = str(dest_path)
                exported_file_count += 1
            representative_files.append(rep_export_path or rep_path)
        ...
        self.cluster_review_path_map = dict(path_map)
        self.cluster_review_info = {
            "exported": True,
            "review_dir": str(review_root.resolve()),
            ...
            "path_remap_count": int(len(path_map)),
            "representative_files_txt": str(rep_txt.resolve()),
        }
        if missing_files:
