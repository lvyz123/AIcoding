...
class LayoutIndex:
    ...
    spatial_index: Any
    effective_pattern_layers: List[Tuple[int, int]] = field(default_factory=list)
    excluded_helper_layers: List[Tuple[int, int]] = field(default_factory=list)
...
def _query_candidate_ids(
    ...
    *,
    ...
    bbox_x0, bbox_y0, bbox_x1, bbox_y1 = (float(value) for value in bbox)
    candidate_ids: List[int] = []
    for elem_id in layout_index.spatial_index.intersection((bbox_x0, bbox_y0, bbox_x1, bbox_y1)):
        idx = int(elem_id)
        if not (
            float(layout_index.bbox_x1[idx]) > bbox_x0
            and float(layout_index.bbox_x0[idx]) < bbox_x1
            and float(layout_index.bbox_y1[idx]) > bbox_y0
            and float(layout_index.bbox_y0[idx]) < bbox_y1
        ):
            continue
...
def _chunk_indices_by_row_width(
    indices: np.ndarray,
    row_width_bytes: int,
    *,
    byte_budget: int = COVERAGE_CHUNK_BYTE_BUDGET,
) -> Iterable[np.ndarray]:
    """按字节预算把目标索引切成小块，避免一次性构造过大的 packed-bitmap 矩阵。"""
...
class MainlineRunner:
    ...
    def __init__(self, *, config: Dict[str, Any], temp_dir: Path, layer_processor: Optional[Any] = None):
        ...
        self.effective_pattern_layers: List[Tuple[int, int]] = []
        self.excluded_helper_layers: List[Tuple[int, int]] = []
    ...
    def _prepare_layout(self, filepath: Path) -> LayoutIndex:
        ...
        marker_polygons: List[gdstk.Polygon] = []
        seen_pattern_layers: Set[Tuple[int, int]] = set()
        ...
        for top_cell in top_cells:
            ...
            for poly in polygons:
                ...
                if layer_spec == self.hotspot_layer_tuple:
                    marker_polygons.append(poly)
                    continue
                seen_pattern_layers.add(layer_spec)
                if self.apply_layer_operations and self.layer_processor is not None:
                    if not self.layer_processor.should_keep_pattern_layer(layer_spec):
                        continue
                pattern_polygons.append(poly)

        if self.apply_layer_operations and self.layer_processor is not None:
            effective_pattern_layers = self.layer_processor.effective_pattern_layers(seen_pattern_layers)
            effective_pattern_set = set(effective_pattern_layers)
            excluded_helper_layers = sorted(
                spec for spec in seen_pattern_layers if spec not in effective_pattern_set
            )
        else:
        ...
        for poly in pattern_polygons:
            ...
            if bbox is None:
                continue
            layer, datatype = _element_layer_datatype(poly)
            element_id = int(len(indexed_elements))
            ...
            spatial_index.insert(element_id, bbox)
        return LayoutIndex(
            ...
            effective_pattern_layers=list(effective_pattern_layers),
            excluded_helper_layers=list(excluded_helper_layers),
        )
    ...
    def _candidate_pair_prefilter(
        self,
        source_candidate: CandidateClip,
        target_candidate: CandidateClip,
    ) -> Tuple[bool, str]:
        """在 candidate 对进入 ACC/ECC 之前执行轻量预筛选。"""
    ...
    def _record_geometry_result(
        ...
        """记录 ACC/ECC 几何匹配结果；默认不做额外统计。"""

        del source_candidate, target_candidate, matched
    ...
    def _evaluate_candidate_coverage_raster(
        ...
        # 保留 candidate 在构建阶段写入的初始 coverage。
        # mainline 默认 base candidate 自覆盖；optimized 的 non-base candidate 会保持为空集。
        for candidate in candidates:
        ...
        for bundle in bundles.values():
            ...
            for source_idx in range(max(0, group_count - 1)):
                ...
                for target_idx in target_indices.tolist():
                    target_candidate = bundle["representatives"][int(target_idx)]
                    prefilter_ok, _ = self._candidate_pair_prefilter(source_candidate, target_candidate)
                    if prefilter_ok:
                        filtered_target_indices.append(int(target_idx))
                if not filtered_target_indices:
                    continue
                target_indices = np.asarray(filtered_target_indices, dtype=np.int64)

                packed_row_bytes = int(bundle["packed"].shape[1]) if bundle["packed"].ndim == 2 else 1
                ...
                if self.matching_mode == "acc":
                    ...
                    for target_chunk in _chunk_indices_by_row_width(target_indices, packed_row_bytes):
                        xor_rows = _bitcount_sum_rows(
                            np.bitwise_xor(bundle["packed"][target_chunk], bundle["packed"][source_idx][None, :])
                        )
                    ...
                    if tol_px <= 0:
                        for target_chunk in _chunk_indices_by_row_width(target_indices, packed_row_bytes):
                            exact_equal = np.all(
                                bundle["packed"][target_chunk] == bundle["packed"][source_idx][None, :],
                                axis=1,
                            )
                        ...
                        for target_chunk in _chunk_indices_by_row_width(target_indices, packed_row_bytes * 4):
                            target_area_limits = ECC_RESIDUAL_RATIO * np.maximum(
                                bundle["areas"][target_chunk].astype(np.float64),
                                1.0,
                            )
                            overlap_indices = target_chunk[
                                (source_area <= bundle["dilated_areas"][target_chunk].astype(np.float64) + source_area_limit)
                                & (
                                    bundle["areas"][target_chunk].astype(np.float64)
                ...
                if matched_chunks:
                    matched_indices = np.concatenate([chunk for chunk in matched_chunks if chunk.size], axis=0)
                else:
    ...
    def _greedy_cover(self, candidates: Sequence[CandidateClip], exact_clusters: Sequence[ExactCluster]) -> List[CandidateClip]:
        ...
        fallback_by_exact_id: Dict[int, CandidateClip] = {}
        for candidate in candidates:
            fallback_by_exact_id.setdefault(int(candidate.origin_exact_cluster_id), candidate)
        selected_ids: Set[str] = set()
        heap: List[Tuple[Tuple[float, ...], str]] = []

        def _priority(candidate: CandidateClip) -> Tuple[float, ...]:
